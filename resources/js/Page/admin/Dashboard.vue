    ...
    <div class="space-y-6 md:space-y-10">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start gap-3 md:gap-4">
        <div>
          ...
          <p class="text-[0.7rem] md:text-[0.75rem] text-white/40">Server Time: <span class="font-mono">{{ serverTime }}</span></p>
        </div>
        <button @click="printReport" class="w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2.5 md:px-6 md:py-3 bg-white/[0.05] border border-white/[0.08] rounded-xl md:rounded-2xl hover:bg-white/10 hover:border-white/20 transition-all group shadow-lg shadow-indigo-500/20 text-xs md:text-base">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        <div class="group p-3 md:p-[30px] rounded-xl md:rounded-[24px] border border-white/[0.08] bg-slate-900/40 transition-all duration-400 hover:scale-[1.02] hover:border-emerald-500/40 hover:bg-slate-900/60 hover:shadow-indigo-500/20 overflow-hidden relative opacity-0 animate-fade-in-up delay-100">
          ...
          <div class="text-xs md:text-[0.9rem] font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2">Sales</div>
          <div class="text-lg md:text-[2.5rem] font-black bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent leading-none tracking-tight mb-1 md:mb-2">
            ₱{{ formatNumber(cards.today_sales) }}
          </div>
          <div v-if="cards.sales_growth !== undefined" class="text-[0.7rem] md:text-sm font-semibold" :class="cards.sales_growth >= 0 ? 'text-emerald-400' : 'text-rose-400'">
            {{ cards.sales_growth >= 0 ? '↑' : '↓' }} {{ Math.abs(cards.sales_growth) }}%
          </div>
          <div class="text-[0.7rem] md:text-[0.8rem] text-slate-500">Final total for today</div>
        </div>
        ...
        <div class="group p-3 md:p-[30px] rounded-xl md:rounded-[24px] border border-white/[0.08] bg-slate-900/40 transition-all duration-400 hover:scale-[1.02] hover:border-violet-500/40 hover:bg-slate-900/60 hover:shadow-indigo-500/20 overflow-hidden relative opacity-0 animate-fade-in-up delay-200">
          <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.05] to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
          <div class="w-9 h-9 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-violet-500/10 flex items-center justify-center text-violet-500 text-base md:text-xl mb-3 md:mb-5">
            <PieChart />
          </div>
          <div class="text-xs md:text-[0.9rem] font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2">Profit</div>
          <div class="text-lg md:text-[2.5rem] font-black bg-gradient-to-r from-violet-400 to-indigo-400 bg-clip-text text-transparent leading-none tracking-tight mb-1 md:mb-2">
            ₱{{ formatNumber(cards.today_profit) }}
          </div>
          <div class="text-[0.7rem] md:text-[0.8rem] text-slate-500">Earnings after cost</div>
        </div>
        ...
        <div class="group p-3 md:p-[30px] rounded-xl md:rounded-[24px] border border-white/[0.08] bg-slate-900/40 transition-all duration-400 hover:scale-[1.02] hover:border-amber-500/40 hover:bg-slate-900/60 hover:shadow-indigo-500/20 overflow-hidden relative opacity-0 animate-fade-in-up delay-300">
          ...
          <div class="text-xs md:text-[0.9rem] font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2">Margin</div>
          <div class="text-lg md:text-[2.5rem] font-black bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent leading-none tracking-tight mb-1 md:mb-2">
            {{ cards.profit_margin }}%
          </div>
          <div class="text-[0.7rem] md:text-[0.8rem] text-slate-500">Profit to sales ratio</div>
        </div>
        ...
        <div class="group p-3 md:p-[30px] rounded-xl md:rounded-[24px] border border-white/[0.08] bg-slate-900/40 transition-all duration-400 hover:scale-[1.02] hover:border-blue-500/40 hover:bg-slate-900/60 hover:shadow-indigo-500/20 overflow-hidden relative opacity-0 animate-fade-in-up delay-400">
          <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.05] to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
          <div class="w-9 h-9 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 text-base md:text-xl mb-3 md:mb-5">
            <ShoppingCart />
          </div>
          <div class="text-xs md:text-[0.9rem] font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2">Orders</div>
          <div class="text-lg md:text-[2.5rem] font-black bg-gradient-to-r from-blue-400 to-sky-400 bg-clip-text text-transparent leading-none tracking-tight mb-1 md:mb-2">
            {{ cards.today_orders }}
          </div>
          <div class="text-[0.7rem] md:text-[0.8rem] text-slate-500">Transactions processed</div>
        </div>
      </div>
      ...
      <div v-if="cards.stale_orders_count > 0" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 md:p-6 bg-orange-500/10 border border-orange-500/20 rounded-xl md:rounded-2xl gap-2 md:gap-3">
        <div class="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h4 class="font-extrabold text-orange-400 text-sm md:text-lg">{{ cards.stale_orders_count }} Stale Orders</h4>
            <p class="text-xs md:text-base text-slate-400">Pending for 24+ hours. Review and update status.</p>
          </div>
        </div>
      ...
      <div class="space-y-3 md:space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
          <GlassCard customClass="lg:col-span-2 p-3 md:p-[30px] rounded-xl md:rounded-[24px]">
          ...
          <div class="p-3 md:p-[30px] rounded-xl md:rounded-[24px] bg-white/[0.03] border border-white/[0.08]">
            ...
            <div v-if="topProducts.length" class="space-y-3 md:space-y-6">
              <div v-for="(product, index) in topProducts" :key="index" class="flex items-center justify-between group">
                <div class="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
                  <span class="text-xs md:text-[0.8rem] font-extrabold text-white/20 group-hover:text-white/40 transition-colors shrink-0">#{{ index + 1 }}</span>
                  <span class="font-bold text-xs md:text-white/90 text-white/80 group-hover:text-white transition-colors truncate">{{ product.product_name }}</span>
                </div>
                <span class="text-[0.7rem] md:text-[0.85rem] font-bold text-emerald-400 shrink-0 ml-2">{{ product.total_sold }} sold</span>
              </div>
            </div>
      ...
      <div class="space-y-3 md:space-y-6" v-once>
        ...
        <div class="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-2 md:gap-3">
          <Link v-for="action in quickActions" :key="action.name" :href="action.path" class="group p-2.5 md:p-4 rounded-xl md:rounded-2xl bg-white/[0.02] border border-white/[0.08] flex flex-col md:flex-row items-center justify-center gap-1 md:gap-3 transition-all duration-300 hover:-translate-y-1 hover:bg-violet-500/10 hover:border-violet-500/30 hover:shadow-[0_10px_20px_rgba(0,0,0,0.2)]">
            ...
            <span class="font-bold text-[0.65rem] md:text-[0.95rem] tracking-tight group-hover:text-violet-400 transition-colors text-center leading-tight">{{ action.name }}</span>
          </Link>
        </div>
      </div>
          ...
          <div v-if="activities.length" class="space-y-2 md:space-y-4">
            <div v-for="(act, index) in activities" :key="index" class="flex items-center justify-between pb-3 md:pb-4 border-b border-white/[0.05] last:border-0 last:pb-0 group">
              <div class="flex items-center gap-3 md:gap-5 min-w-0 flex-1">
                <div :style="{ backgroundColor: act.color + '20', color: act.color }" class="w-9 h-9 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center text-base md:text-xl transition-transform group-hover:scale-110 shrink-0">
                  <i :class="['fas', act.icon]"></i>
                </div>
                <div class="min-w-0">
                  <h5 class="font-bold text-xs md:text-[0.95rem] text-white group-hover:text-violet-300 transition-colors truncate">{{ act.title }}</h5>
                  <p class="text-[0.7rem] md:text-[0.85rem] text-white/50 truncate">{{ act.desc }}</p>
                </div>
              </div>
              <div class="text-right shrink-0 ml-2">
                <span class="text-[0.65rem] md:text-[0.75rem] text-white/30 font-bold uppercase tracking-widest">{{ formatTime(act.time) }}</span>
              </div>
            </div>
          </div>
