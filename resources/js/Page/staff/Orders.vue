      ...
      <GlassCard customClass="overflow-hidden border-white/10">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-separate border-spacing-y-2 px-4">
            ...
            <tbody>
              <tr v-for="order in localOrders" :key="order.id" class="bg-white/[0.03] hover:bg-white/[0.06] transition-all group">
              ...
              <tr v-if="localOrders.length === 0">
                <td colspan="6" class="px-6 py-32 text-center">
                  <div class="flex flex-col items-center gap-4 opacity-10">
                    ...
                    <p class="font-bold text-lg italic">No orders found.</p>
                  </div>
                </td>
              </tr>
            </tbody>
...
import { ChevronLeft, Eye, X, PackageOpen, Flame, Truck, CheckCheck } from 'lucide-vue-next';
...
const viewDetails = async (order) => {
  ...
  try {
    ...
    if (response.data.status === 'success') {
      selectedOrder.value.items = response.data.data;
    }
  } catch (error) {
