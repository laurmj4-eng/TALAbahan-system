  ...
  <div class="p-6 md:p-8 space-y-8">
    ...
    <div v-if="filteredOrders.length > 0" class="space-y-4">
      <GlassCard 
        ...
        :key="order.id"
        customClass="p-6 border-white/10 hover:border-indigo-500/30 hover:bg-white/[0.04] transition-all group"
      >
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div class="flex flex-col items-end gap-4">
            <div class="text-2xl font-black text-emerald-400">₱{{ formatNumber(order.total_amount) }}</div>
            <div class="flex flex-wrap items-center gap-3">
              ...
              <div class="flex gap-2">
                <button @click="viewDetails(order)" class="p-3 bg-white/5 border border-white/10 rounded-xl text-indigo-400 hover:bg-indigo-500/20 hover:border-indigo-500/40 transition-all flex items-center gap-2">
                  <Eye class="w-4 h-4" />
    ...
    <div v-else class="flex flex-col items-center justify-center py-32 space-y-6 opacity-20">
      ...
      <router-link to="/customer/dashboard" class="px-8 py-3 bg-white text-indigo-900 rounded-2xl font-black text-sm hover:bg-indigo-50 transition-all">
        Go to Shop
      </router-link>
    </div>
        ...
        <div class="space-y-8">
          ...
          <div class="space-y-4">
            <div v-for="item in selectedOrder.items" :key="item.id" class="flex justify-between items-center py-4 border-b border-white/5 last:border-0">
              ...
              <div class="text-emerald-400 font-black">₱{{ formatNumber(item.subtotal) }}</div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-3 pt-4">
            <button v-if="selectedOrder.can_pay_now" @click="payNow(selectedOrder.id)" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center gap-2 transition-all">
